...

{
    ...
    public function survey()
    {
        $data = array();
        $this->load->view('layout/header', $data);
        $this->load->view('admin/survey/create_survey', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function all_survey()
    {
        ...
        $this->db->select('*')
                 ->from('survey')
                 ->join('survey_assign', 'survey.id = survey_assign.survey_id');
        $result = $this->db->get();
        $res=$result->result_array();
        $data['hostelroomlist'] = $res;
        // print_r($res);exit;

        $this->load->view('layout/header', $data);
        $this->load->view('admin/survey/all_survey', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    {  
        ...
        $data = array(  
                        ...
                        'survey_discription'     =>           $this->input->post('survey_discription'),  
                        'question'               =>           $this->input->post('question'),  
                        'options'                =>           implode(",",$this->input->post('options'))
                        // 'visible'                =>           implode(",",$this->input->post('visible')) 
                        );  
        $data1 = array(  
                'visible'                =>           implode(",",$this->input->post('visible')) 
                ); 
        ...
        $this->load->model('Survey_model');
        $this->Survey_model->add_survey('survey',$data,$data1);
        // $this->Survey_model->getSetting();
   //      $insert_id = $this->db->insert_id();  
   //      $insert_id = $this->db->insert_id();
   // echo $insert_id;exit;
        redirect("admin/all-survey");  
    } 

    public function survey_view()
    {

      if (!$this->rbac->hasPrivilege('survey_view', 'can_view1')) {
            access_denied();
        }

        $data = array();
        $this->load->view('layout/header', $data);
        $this->load->view('admin/survey/survey_view', $data);
        $this->load->view('layout/footer', $data);
    }
