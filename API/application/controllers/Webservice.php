...
//===============
    ...
    public function getNotice() //getNotifications()
    {
        $method = $this->input->server('REQUEST_METHOD');
        if ($method != 'POST') {
            json_output(400, array('status' => 400, 'message' => 'Bad request.'));
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    $params = json_decode(file_get_contents('php://input'), true);
                    $type   = $params['type'];
                    $resp   = $this->webservice_model->getNotice($type);
                    json_output($response['status'], $resp);
                }
            }
        }
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
					...
					$new_arr = array();
					foreach ($days as $tm_key => $tm_value) {
						$new_date = date('Y-m-d', strtotime($date_from . ' +' . $day_count_timetable . ' day'));
						$data = array('date' => $new_date, 'subject_group_class_section_id' => $result[0]->subject_group_class_section_id);
						$syllabus_data = $this->syllabus_model->get_subject_syllabus_student_byDate($data);
						
						$new_arr[$new_date] = $syllabus_data;
						$day_count_timetable++;
					}
					$data['timetable'] = $new_arr;
					$data['status']    = "200";
                    json_output($response['status'], $data);
                }
            }
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    // $student_discount_fee['discount'] = true;
        ...
        if ($method != 'POST') {
            ...
            if ($check_auth_client == true) {
                ...
                if ($response['status'] == 200) {
                    ...
                    $this->form_validation->set_rules('first_doc', 'File', 'callback_handle_upload_file_compulsory');  
                    if ($this->form_validation->run() == false) { 
                    ...
                    } else { // die($this->config->item('upload_path'));
                        //==================
                        ...
                        //     $img_name  = $upload_path . $fileInfo['extension'];
