...
class Homework_model extends CI_Model
{
    ...
    public function getStudentHomework($class_id, $section_id, $student_session_id)
    {
        ...
        $this->db->select("`homework`.*, `subjects`.`name`, `sections`.`section`, `classes`.`class`, `create_staff`.`name` as `staff_created`, IFNULL(`evaluate_staff`.`name`,'') as `staff_evaluated`,IFNULL( homework_evaluation.id, '') as homework_evaluation_id");
